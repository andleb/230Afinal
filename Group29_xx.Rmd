...
* **Virtual countries**: To account for the arbitrary nature of some political boundaries with respect to ethnolinguistic
  groupings, the world is split into arbitrary *virtual countries* and the regression performed again.
...
Finally, in regions which have undergone a major demographic shift in the last 500 years, the explanatory power of the model
...
languages comes from the *World Language Mapping System*.
...
```{r fig1, fig.cap="\\label{fig:suit} Land quality for agriculture across countries", echo=F, cache=T}
...
This was done by intersecting the cells shown in the previous figure with country boundaries, and then plotting a KDE with the Epanechnikov kernel.
